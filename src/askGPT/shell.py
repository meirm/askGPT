...
class Shell(cmd.Cmd):
    ...
    def do_dream(self, args):
        ...
        with console.status("waiting for response ...", spinner="dots"):
            url = self._config.chat.dream(prompt)
            if url:
                ...
                filepath = os.path.join(self._config.conversations_path, filename)
    ...
    def do_capabilities(self, args):
        ...
        if len(args) == 0:
            ...
            for capability in sorted(self.capabilities):
                print(f"{capability}: {self.capabilities[capability].description}")
        elif len(args) == 1:
            if args[0] == "enable":
